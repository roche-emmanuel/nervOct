...
% [J grad] = nn_cost_function_cuda(nn_params, layer_sizes, X, yy, lambda)

% ==> Should throw an error if invalid count of arguments:
%!error <nn_cost_function_cuda: Invalid number of arguments: 1> nn_cost_function_cuda(1)

% ==> Should throw an error if invalid type of arguments:
%!error <nn_cost_function_cuda: nn_params \(arg 0\) should be a matrix type> nn_cost_function_cuda(1,2,3,4,5)
%!error <nn_cost_function_cuda: layer_sizes \(arg 1\) should be a matrix type> nn_cost_function_cuda(rand(2,2),2,3,4,5)
%!error <nn_cost_function_cuda: X \(arg 2\) should be a matrix type> nn_cost_function_cuda(rand(2,2),[3 2 1],3,4,5)
%!error <nn_cost_function_cuda: yy \(arg 3\) should be a matrix type> nn_cost_function_cuda(rand(2,2),[3 2 1],rand(3,3),4,5)
%!error <nn_cost_function_cuda: lambda \(arg 4\) should be a double> nn_cost_function_cuda(rand(2,2),[3 2 1],rand(3,3),rand(4,5),false)
...
%!	size(X)
%!	size(yy)
%!	costFunc = @(p) nn_cost_function_cuda(p, lsizes, X, yy, lambda);
%!	
%!	num=10;
%!	profile on;
%!	tic()
