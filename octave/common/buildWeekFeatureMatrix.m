function X =  buildWeekFeatureMatrix(data,cfg)
...
%!error <must provide a dataset as input> buildWeekFeatureMatrix()

% ==> We need a valid configuration for this method:
%!error <must provide a configuration> buildWeekFeatureMatrix(rand(5,4))
...
%!error <Invalid number of rows in week dataset> buildWeekFeatureMatrix(rand(5,4),config())
%!error <Invalid minute indices in dataset> buildWeekFeatureMatrix(rand(config().num_week_minutes,4),config())
...
%!  X = buildWeekFeatureMatrix(mat,cfg);
%!
%!  X;
%!  len = sum(abs(X-Xe));
%!  if len>0,
