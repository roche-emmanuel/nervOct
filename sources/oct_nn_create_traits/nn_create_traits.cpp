           ...
           "nn_create_traits function providing C++ implementation of neural network cost function")
{
  ...
  CHECK_RET(X_train.dim2() == y_train.dim2(), "nn_create_traits: mismatch in nsamples_train: " << X_train.dim2() << "!=" << y_train.dim2());
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_double_type(), "nn_create_traits: spaeBeta is not a double type");
    CHECK_RET(0.0 <= val.double_value(), "nn_create_traits: out of range spaeBeta value");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_double_type(), "nn_create_traits: spaeSparsity is not a double type");
    CHECK_RET(0.0 < val.double_value() && val.double_value() < 1.0, "nn_create_traits: out of range spaeSparsity value");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_bool_type(), "nn_create_traits: useSoftmax is not a bool type");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_double_type(), "nn_create_traits: costMode is not a double type");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_bool_type(), "nn_create_traits: debug is not a bool type");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_double_type(), "nn_create_traits: bias is not a double type");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_double_type(), "nn_create_traits: lambda is not a double type");
  ...
  if (val.is_defined())
  {
    CHECK_RET(val.is_matrix_type(), "nn_create_traits: dropouts is not a matrix type");
    Matrix drop_mat = val.matrix_value();
    CHECK_RET(drop_mat.numel() == nt, "nn_create_traits: invalid size for dropout matrix size: " << drop_mat.numel() << "!=" << nt);

    dropouts = new double[nt];
    for (unsigned int i = 0; i < nt; ++i)
    {
      dropouts[i] = drop_mat(i);
      CHECK_RET(dropouts[i] >= 0.0 && dropouts[i] <= 1.0, "nn_create_traits: dropout for layer " << i << " is out of range");
    }
  ...
  CHECK_RET(id>0, "ERROR: exception occured in nn_create_traits.")
