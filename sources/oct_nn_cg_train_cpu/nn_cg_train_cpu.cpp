...
public:
  NERVManager() {
    ...
    if(!_cgtrain) {
      error("ERROR: cannot find cgtrainCPU method! err=%d",GetLastError());
    }
  }
...
DEFUN_DLD (nn_cg_train_cpu, args, nargout,
           "nn_cg_train_cpu function providing C++ implementation of nn_cg_train")
{
  octave_value_list result;

  // we ecpect to receive 5 arguments:
  int nargin = args.length();
  CHECK(nargin==6,"nn_cg_train_cpu: Invalid number of arguments: " << nargin);

  // Check the argument types:
  CHECK(args(0).is_matrix_type(),"nn_cg_train_cpu: layer_sizes (arg 0) should be a matrix type");
  CHECK(args(1).is_matrix_type(),"nn_cg_train_cpu: X (arg 1) should be a matrix type");
  CHECK(args(2).is_matrix_type(),"nn_cg_train_cpu: yy (arg 2) should be a matrix type");
  CHECK(args(3).is_matrix_type(),"nn_cg_train_cpu: init_params (arg 3) should be a matrix type");
  CHECK(args(4).is_double_type(),"nn_cg_train_cpu: lambda (arg 4) should be a double");
  CHECK(args(5).is_double_type(),"nn_cg_train_cpu: maxiter (arg 5) should be a double");
