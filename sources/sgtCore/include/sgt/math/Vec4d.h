...
class Vec4d
{
    public:
        ...
        inline bool isNaN() const { return sgt::isNaN(_v[0]) || sgt::isNaN(_v[1]) || sgt::isNaN(_v[2]) || sgt::isNaN(_v[3]); }
...
inline std::ostream& operator<<(std::ostream& os, const Vec4d& val)
{
    os << "Vec4d("<<val.x()<<", "<<val.y()<<", "<<val.z()<<", "<<val.w()<<")";
    return os;
}
