 ...
 * the terms of the OpenSceneGraph Public License (SGTPL) version 0.0 or
...
#ifndef SGT_MATRIXF
#define SGT_MATRIXF 1

#include <sgtcore.h>
#include <sgt/Object.h>
#include <sgt/math/Vec3d.h>
#include <sgt/math/Vec4d.h>
#include <sgt/math/Quat.h>
...
class SGTCORE_EXPORT Matrixf
{
    public:
        ...
        inline bool isNaN() const { return sgt::isNaN(_mat[0][0]) || sgt::isNaN(_mat[0][1]) || sgt::isNaN(_mat[0][2]) || sgt::isNaN(_mat[0][3]) ||
                                                 sgt::isNaN(_mat[1][0]) || sgt::isNaN(_mat[1][1]) || sgt::isNaN(_mat[1][2]) || sgt::isNaN(_mat[1][3]) ||
                                                 sgt::isNaN(_mat[2][0]) || sgt::isNaN(_mat[2][1]) || sgt::isNaN(_mat[2][2]) || sgt::isNaN(_mat[2][3]) ||
                                                 sgt::isNaN(_mat[3][0]) || sgt::isNaN(_mat[3][1]) || sgt::isNaN(_mat[3][2]) || sgt::isNaN(_mat[3][3]); }
        ...
        void decompose( sgt::Vec3f& translation,
                        sgt::Quat& rotation,
                        sgt::Vec3f& scale,
                        sgt::Quat& so ) const;

        /** decompose the matrix into translation, rotation, scale and scale orientation.*/
        void decompose( sgt::Vec3d& translation,
                        sgt::Quat& rotation,
                        sgt::Vec3d& scale,
                        sgt::Quat& so ) const;
...
class RefMatrixf : public Object, public Matrixf
{
    public:
        ...
        RefMatrixf( const RefMatrixf& other) : Matrixf(other) {}
...
{
    return scale((value_type)v.x(), (value_type)v.y(), (value_type)v.z() );
}
...
{
    return translate((value_type)v.x(), (value_type)v.y(), (value_type)v.z() );
}
...
{
    Vec3d::value_type d = 1.0/(_mat[3][0]*v.x()+_mat[3][1]*v.y()+_mat[3][2]*v.z()+_mat[3][3]) ;
    return Vec3d( (Vec3d::value_type)((_mat[0][0]*v.x() + _mat[0][1]*v.y() + _mat[0][2]*v.z() + _mat[0][3])*d),
        (Vec3d::value_type)((_mat[1][0]*v.x() + _mat[1][1]*v.y() + _mat[1][2]*v.z() + _mat[1][3])*d),
        (Vec3d::value_type)((_mat[2][0]*v.x() + _mat[2][1]*v.y() + _mat[2][2]*v.z() + _mat[2][3])*d)) ;
}
...
{
    Vec3d::value_type d = 1.0f/(_mat[0][3]*v.x()+_mat[1][3]*v.y()+_mat[2][3]*v.z()+_mat[3][3]) ;
    return Vec3d( (Vec3d::value_type)((_mat[0][0]*v.x() + _mat[1][0]*v.y() + _mat[2][0]*v.z() + _mat[3][0])*d),
        (Vec3d::value_type)((_mat[0][1]*v.x() + _mat[1][1]*v.y() + _mat[2][1]*v.z() + _mat[3][1])*d),
        (Vec3d::value_type)((_mat[0][2]*v.x() + _mat[1][2]*v.y() + _mat[2][2]*v.z() + _mat[3][2])*d));
}
...
inline void Matrixf::preMultTranslate( const Vec3d& v )
{
    for (unsigned i = 0; i < 3; ++i)
    {
        ...
        if (tmp == 0)
            continue;
        _mat[3][0] += (value_type)(tmp*_mat[i][0]);
        _mat[3][1] += (value_type)(tmp*_mat[i][1]);
        _mat[3][2] += (value_type)(tmp*_mat[i][2]);
        _mat[3][3] += (value_type)(tmp*_mat[i][3]);
    }
}
...
inline void Matrixf::postMultTranslate( const Vec3d& v )
{
    for (unsigned i = 0; i < 3; ++i)
    {
        ...
        if (tmp == 0)
            continue;
        _mat[0][i] += (value_type)(tmp*_mat[0][3]);
        _mat[1][i] += (value_type)(tmp*_mat[1][3]);
        _mat[2][i] += (value_type)(tmp*_mat[2][3]);
        _mat[3][i] += (value_type)(tmp*_mat[3][3]);
    }
}
...
{
    _mat[0][0] *= (value_type)v[0]; _mat[0][1] *= (value_type)v[0]; _mat[0][2] *= (value_type)v[0]; _mat[0][3] *= (value_type)v[0];
    _mat[1][0] *= (value_type)v[1]; _mat[1][1] *= (value_type)v[1]; _mat[1][2] *= (value_type)v[1]; _mat[1][3] *= (value_type)v[1];
    _mat[2][0] *= (value_type)v[2]; _mat[2][1] *= (value_type)v[2]; _mat[2][2] *= (value_type)v[2]; _mat[2][3] *= (value_type)v[2];
}
...
{
    _mat[0][0] *= (value_type)v[0]; _mat[1][0] *= (value_type)v[0]; _mat[2][0] *= (value_type)v[0]; _mat[3][0] *= (value_type)v[0];
    _mat[0][1] *= (value_type)v[1]; _mat[1][1] *= (value_type)v[1]; _mat[2][1] *= (value_type)v[1]; _mat[3][1] *= (value_type)v[1];
    _mat[0][2] *= (value_type)v[2]; _mat[1][2] *= (value_type)v[2]; _mat[2][2] *= (value_type)v[2]; _mat[3][2] *= (value_type)v[2];
}
...
inline std::ostream& operator<<(std::ostream& os, const Matrixf& mat)
{
    os << "Matrixf(\n";
