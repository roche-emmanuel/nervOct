...
*/
std::ostream& LogRecord::GetStream(int level, const std::string& filename, int line, const std::string& trace) {
	_level = level;
	_trace = trace;
	...
	_os << "\t" << std::string((level > LogManager::SEV_NOTICE ? level - LogManager::SEV_NOTICE : 0)*3, ' ');
