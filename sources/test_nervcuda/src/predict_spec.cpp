...
BOOST_AUTO_TEST_CASE( test_nn_predict_with_softmax )
{
  typedef double value_type;
  value_type epsilon = std::numeric_limits<value_type>::epsilon();
...
BOOST_AUTO_TEST_CASE( test_nn_predict_with_dropout_and_softmax )
{
  typedef double value_type;
  value_type epsilon = std::numeric_limits<value_type>::epsilon();
  ...
  for (unsigned int i = 0; i < num; ++i)
  {
    ...
    for (unsigned int j = 0; j < ny; ++j)
    {
      value_type v1 = hx[j];
      value_type v2 = pred_hx[j];
      BOOST_CHECK_MESSAGE(abs(v1 - v2) <= 100 * epsilon, "Mismatch (with dropout and softmax) on hx element " << j << ": " << v1 << "!=" << v2);
    }
